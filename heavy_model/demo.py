"""
Heavy Model Demo — Unified Pipeline (YOLOv8 + Swin-B + LSTM-SORT)
=================================================================
...
#  CONFIGURATION
# ══════════════════════════════════════════════════════════════════════════════
DETECTOR_PATH    = "detector/best.pt"
CLASSIFIER_PATH  = "classifier/best_swin_b.pth"
TELEMETRY_JSON   = None   # path to instances JSON, or None
...
SHOW_PREDICTIONS     = True   # LSTM predicted box (next frame)
...
def load_telemetry(json_path, folder_name):
    try:
        ...
        print(f"  [telemetry] Loaded {len(tmap)} frames for video_id={video_id}.")
        return tmap
    except Exception as e:
        print(f"  [telemetry] Failed ({e}) — zero telemetry."); return {}

def render_frame(frame, active_tracks, yolo_boxes, tracker, colors,
                 is_coast_frame, show_raw, show_pred, show_trail, trail_len):
    """Draw tracking overlays. Coast frames get a tinted border."""
    if is_coast_frame:
        h, w = frame.shape[:2]
    ...
    if show_raw and not is_coast_frame:
    ...
    mode_str = "COAST (LSTM)" if is_coast_frame else "DETECT"
    mode_col = (0, 140, 255) if is_coast_frame else (100, 220, 100)
...
def run(input_path: str, output_path: str = None, skip_n: int = 3):
    ...
    if USE_CLASSIFIER:
        print("  [models] Loading classifier (Swin-B) ...")
        classifier = timm.create_model('swin_base_patch4_window7_224', num_classes=NUM_CLASSES)
        ckpt = torch.load(str(HERE / CLASSIFIER_PATH), map_location=DEVICE)
        classifier.load_state_dict(ckpt['model_state_dict'])
        classifier.to(DEVICE).eval()
        idx_to_class = ckpt['idx_to_class']
        vit_transform = T.Compose([T.Resize((224, 224)), T.ToTensor(),
            T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])

    print("  [models] Loading LSTM-SORT tracker ...")
    ...
    if output_path is None:
        ...
        v = 1
        while (out_dir / f"heavy_{mode_lbl}_{stem}_v{v}.mp4").exists(): v += 1
        output_path = str(out_dir / f"heavy_{mode_lbl}_{stem}_v{v}.mp4")
        
    writer = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'mp4v'),
                             OUTPUT_FPS, (w_vid, h_vid))

    print(f"\n  [output] {output_path}  ({w_vid}x{h_vid} @ {OUTPUT_FPS} fps)")
    print(f"  [config] SAHI={USE_SAHI}  Classifier={USE_CLASSIFIER}  "
          f"skip_N={skip_n}  max_age={TRACKER_MAX_AGE}\n")
    ...
    for frame_idx, img_path in enumerate(images[:n_frames]):
        ...
        # If skip_n == 1, is_coast is always False (Baseline mode)
        ...
        if not is_coast:
            ...
            for bx, by, bw, bh, cls_name, score in raw_boxes:
                if USE_CLASSIFIER and classifier and score < CLASSIFIER_THRESH:
                    x1, y1 = max(0, int(bx)), max(0, int(by))
                    x2, y2 = min(w_vid, int(bx+bw)), min(h_vid, int(by+bh))
                    if x2 > x1 and y2 > y1:
                        with torch.no_grad():
                            out = classifier(vit_transform(pil_img.crop((x1,y1,x2,y2))).unsqueeze(0).to(DEVICE))
    ...
    print(f"  [stats] Detection frames: {detect_count}/{total} "
          f"({100*detect_count/total:.1f}%)")
    if total > 0 and coast_count > 0:
        print(f"          Coast frames    : {coast_count}/{total} "
              f"({100*coast_count/total:.1f}%)")

# ══════════════════════════════════════════════════════════════════════════════
if __name__ == '__main__':
    DEFAULT_INPUT = r"C:\Users\youse\Downloads\Helwan\3rd\Neural Network\Seedrone\data\DJI_0063_images.tar.gz"
    
    parser = argparse.ArgumentParser(description="Heavy Model Tracker Demo (Baseline & Sparse modes).")
    ...
    parser.add_argument("--skip-n", type=int, default=3, 
                        help="Run object detector only every Nth frame. The LSTM coasts in-between. "
    ...
    print(f"\n{'='*60}")
    print(f"  Heavy Model Demo — {mode_lbl} Mode")
