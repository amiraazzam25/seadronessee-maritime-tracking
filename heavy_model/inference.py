"""
Heavy Model Inference API
=========================
Provides a programmatic interface to run the heavy pipeline (YOLOv8 + SwinB + LSTM-SORT)
...
class HeavyInferencePipeline:
    def __init__(self, 
        ...
        # Load Detector
        print("[Heavy API] Loading detector...")
        ...
        if use_classifier:
            print("[Heavy API] Loading Swin-B classifier...")
            try:
                self.classifier = timm.create_model('swin_base_patch4_window7_224', num_classes=5)
                ...
                print(f"[Heavy API] WARNING: Failed to load classifier: {e}")
                self.use_classifier = False
                
        # Load Tracker
        print("[Heavy API] Initializing tracker...")
    ...
    def process_frame(self, frame_bgr, telemetry=None, is_coast_frame=False):
        ...
        if is_coast_frame:
            # Sparse frame: tracker coasts using NN motion predictions only
            return self.tracker.update_coast(telemetry)
            
        # Convert to RGB and PIL for consistent pre-processing
